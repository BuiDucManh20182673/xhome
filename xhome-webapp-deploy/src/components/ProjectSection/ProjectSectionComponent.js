...
import { mdiDelete, mdiClipboardEditOutline, mdiContentCopy } from '@mdi/js';
...
const mapDisptachToPropsProjectManager = (dispatch) => {
    return ({
        ...
        searchProjectManager: (data, isActive) => {
            dispatch(actions.searchProjectManagerRequest(data, isActive))
        },
        deleteProjectManager: (data) => {
            dispatch(actions.deleteProjectManagerRequest(data))
        },
...
const ProjectSectionComponent = connect(mapStateToPropsProjectManager, mapDisptachToPropsProjectManager)((props) => {
    useEffect(() => { props.getProjectManager(1, 1) }, []);
                ...
                {listData.map((item, idx) => {
                    if (item.isActive && item.isActive) {
                        return <Item key={idx} item={item}
                            deleteProjectManager={props.deleteProjectManager} chooseProjectToUpdate={props.chooseProjectToUpdate} copyProject={props.copyProject} />
                    } else {
                        return <></>
                    }
                })}
                            ...
                            onPaginate={(pageIndex) => {
                                props.textSearch !== "" ?
                                    props.searchProjectManager({
                                        ...
                                        isActive: 1
                                    }) : props.getProjectManager(pageIndex, 1)
...
const Item = (props) => {
    ...
    return (
        <>
            {
                !isEditing
                    ? <RowContent properties={properties} item={props.item} deleteProjectManager={props.deleteProjectManager}
                    ...
                    <NavLink to="/dashboard/project-creation" >
                        <Button className={'btn btn-edit'}
                            ...
                            onClick={() => {
                                ...
                                props.chooseProjectToUpdate({ item: props.item, typeEditor: "editProject" })
                            }}
                        >
                            <Icon path={mdiClipboardEditOutline} size={1} />
                        </Button>
                    </NavLink>
                    <Button className="btn btn-delete" onClick={() => setOpenModalConfirmDelete(true)}>
                    ...
                    >
                        {isAdmin === 1 ?
                            <div style={{ padding: '40px 50px', backgroundColor: '#3327281c', textAlign: "center" }}>
                                <p>Xác nhận xóa dự án!</p>
                                <Button
                                    color="secondary"
                                    style={{ padding: '10px 20px', backgroundColor: '#3327281c' }}
                                    onClick={() => { props.deleteProjectManager(props.item) }}
                                > Xóa
                                </Button>
                                <Button
